...
/**
 * 检查哪吒面板进程 (NZ-Agent) 是否运行
 * @param {function} callback - 回调函数，参数为布尔值，true 表示进程正在运行，false 表示未运行
 */
function checkNezhaAgentProcess(callback) {
    const checkCommand = 'ps aux | grep nezha-agent | grep -v grep'; // 排除 grep 自身进程
    exec(checkCommand, (error, stdout, stderr) => {
        if (error) {
            logMessage(`检查 NZ-Agent 进程状态失败: ${error.message}`); // 日志信息简化
            callback(false); // 检查失败，默认为进程未运行，避免误操作
            return;
        }
        if (stdout) {
            // 如果 stdout 有输出，说明进程正在运行
            logMessage("NZ-Agent 正在运行。"); // 日志信息简化
            callback(true);
        } else {
            // stdout 为空，进程未运行
            logMessage("NZ-Agent 未运行。"); // 日志信息简化
            callback(false);
        }
    });
}
...
function restartNezhaAgentProcess() {
    ...
    executeCommand(restartCommand, "重启 NZ-Agent 进程", false, (output) => { // actionName 简化
        if (output) {
            logMessage("NZ-Agent 进程重启成功。"); // 日志信息简化
        } else {
            logMessage("NZ-Agent 进程重启失败，请检查日志。"); // 日志信息简化
        }
    });
}

/**
 * 定时监控哪吒面板进程 (NZ-Agent)，如果未运行则自动重启
 */
function monitorNezhaAgent() {
    checkNezhaAgentProcess((isRunning) => {
        if (!isRunning) {
            logMessage("检测到 NZ-Agent 未运行，尝试自动重启..."); // 日志信息简化
...
setInterval(monitorNezhaAgent, 600000); // 1800000 毫秒 = 30 分钟
